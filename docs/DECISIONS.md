...
## 2026-09-26 · Reminders, notifications without Google, prescription photos, receipts

- **Reminders come only from the pharmacist's decision** (times per day, days). The patient can move a time; they can't add doses. Each notification shows the pharmacist's own instructions, never text the app or the AI makes up.
- **Scheduled on the phone**: the next 60 doses as local notifications (`flutter_local_notifications`, inexact while idle, so no exact-alarm permission is needed), topped up at every start and background round. Times are scheduled as UTC instants of the phone's local time (Syria has no DST since 2022). `timezone` comes with the notifications package.
